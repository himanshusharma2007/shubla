...
const checkRoomAvailability = async (quantity, checkIn, checkOut, roomType, guests = 1) => {
    ...
    try {
        ...
        const bookedRooms = activeBookings.reduce((total, booking) => total + booking.quantity, 0);
        ...
        console.log('Potential checkouts before checkin:', checkoutBeforeCheckin.length);
...
const checkCampAvailability = async (quantity, checkIn, checkOut, guests = 1) => {
    try {
        ...
        const bookedCamps = activeBookings.reduce((total, booking) => total + booking.quantity, 0);
        ...
        const price = (checkInDay === 0 || checkInDay === 6) ? camp.pricing.weekend : camp.pricing.weekday;
...
const checkParkingAvailability = async (quantity, checkIn, checkOut) => {
    try {
        ...
        const bookedSlots = activeBookings.reduce((total, booking) => total + booking.quantity, 0);
        const availableSlots = parkingSlot.totalSlots - bookedSlots;

        // Calculate price based on weekday/weekend
        const checkInDay = new Date(checkIn).getDay();
        const price = (checkInDay === 0 || checkInDay === 6) ? parkingSlot.pricing.weekend : parkingSlot.pricing.weekday;
...
const checkAvailability = async (serviceType, quantity, checkIn, checkOut, options = {}) => {
    const { roomType, guests } = options;

    switch (serviceType) {
        case "room":
            return checkRoomAvailability(quantity, checkIn, checkOut, roomType, guests);
        case "camp":
